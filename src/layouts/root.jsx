...
function PlayerHand({ game, id, position }) {
  const cards = game.getPlayerHand(id);
  const isHorizontal = position === "bottom" || position === "top";
  
  let containerClass = "absolute flex z-10 ";
  if (position === "bottom") containerClass += "flex-row bottom-4 left-1/2 -translate-x-1/2 pb-10";
  if (position === "top") containerClass += "flex-row top-4 left-1/2 -translate-x-1/2 pt-10";
  if (position === "left") containerClass += "flex-col left-4 top-1/2 -translate-y-1/2 pl-10";
  if (position === "right") containerClass += "flex-col right-4 top-1/2 -translate-y-1/2 pr-10";
  ...
  return (
    <div className={containerClass}>
          ...
          {!passesSent && (
            <button
              ...
              className={clsx(
                ...
                canSend ? "bg-blue-500 text-white hover:bg-blue-600 cursor-pointer" : "bg-gray-300 text-gray-500"
              )}
            >
              Send Passes
            </button>
      ...
      {cards.map((card, idx) => {
        ...
        let overlapClass = "";
        if (!isFirst) {
          if (isHorizontal) overlapClass = "-ml-28";
        ...
        let transformClass = "";
        if (selected) {
           if (position === "bottom") transformClass = "-translate-y-8";
           if (position === "top") transformClass = "translate-y-8";
           if (position === "left") transformClass = "translate-x-8";
           if (position === "right") transformClass = "-translate-x-8";
        }

        return (
          <div key={card + idx} className={clsx(overlapClass, "transition-transform", transformClass, !enabled && "opacity-60")}>
            <Card
              onClick={() => {
                if (game.state === "pass") {
                  if (!passesSent) {
...
function GameTable({ game }) {
  ...
  return (
    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-green-800 rounded-full border-8 border-green-900 shadow-2xl flex items-center justify-center">
      <div className="relative w-full h-full">
        ...
        {trick && trick['1'] && (
          <div className="absolute left-4 top-1/2 -translate-y-1/2 z-10">
             <Card card={trick['1']} className="shadow-lg transform scale-75 rotate-90" />
          </div>
        )}
        {trick && trick['2'] && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10">
            <Card card={trick['2']} className="shadow-lg transform scale-75 rotate-180" />
          </div>
        )}
        {trick && trick['3'] && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 z-10">
             <Card card={trick['3']} className="shadow-lg transform scale-75 -rotate-90" />
          </div>
        )}
      </div>
    </div>
...
export default function Root({ children }) {
  ...
  return (
    <div className="bg-gray-900 flex items-center justify-center w-screen h-screen relative overflow-hidden font-sans">
      ...
      <PlayerHand game={game} id={1} position="left" />
      <PlayerHand game={game} id={2} position="top" />
      <PlayerHand game={game} id={3} position="right" />
    </div>
  );
}
